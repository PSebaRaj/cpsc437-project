  ...
  return (
    <div>
      ...
      <div className="table-header">
        <img width={1000} height={100} src={require('./state1.jpeg')} alt="State 1" />
        {beta !== 0 &&
        <div>
          <h2>beta = {beta}</h2>
          <img width={200} height={100} src={require('./state2.jpeg')} alt="State 2" />
        </div>
        }
      </div>
      }
      {(region === "county") &&
      <div className="table-header">
        <img width={1000} height={100} src={require('./county1.jpeg')} alt="County 1" />
        {beta !== 0 &&
        <div>
          <h2>beta = {beta}</h2>
          <img width={200} height={100} src={require('./county2.jpeg')} alt="County 2" />
        </div>
        }
      </div>
      }
      <h3>Note: Columns with years 1970-2020 represent percent of adult residents with a bachelor's degree or higher</h3>
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          <TableHead>
            <StyledTableRow>
              <StyledTableCell align="center">Name</StyledTableCell>
              {(region === "state" || region === "county") && <StyledTableCell align="center">Lambda</StyledTableCell>}
          ...
          <TableBody>
            {data.map((row) => (
              <StyledTableRow key={row.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                {(region === "state" || region === "county") && <StyledTableCell align="center">{(row.medianHHIncome/row.y2020)/100000}</StyledTableCell>}
